...
def main():
  ...
  tracked_tubes = {}
  alpha = 0.2  # Smoothing factor for tracking
  ...
  while True:
    ...
    for x, y, w, h in current_detected_tubes:
      ...
      if matched_id in tracked_tubes:
        ...
        smooth_x = int(alpha * x + (1 - alpha) * ox)
        smooth_y = int(alpha * y + (1 - alpha) * oy)
        smooth_w = int(alpha * w + (1 - alpha) * ow)
        smooth_h = int(alpha * h + (1 - alpha) * oh)
      else:
      ...
      if tube_roi.size > 0:
        ...
        if len(row_val) > 15:
          ...
          # --- STABILIZATION LOGIC ---
          # Collect sample positions during the warm-up phase to find the best steady position
          if matched_id in tracked_tubes:
            tube_data = tracked_tubes[matched_id]
            samples = tube_data["samples"]
            samples.append(interface_global_y)

            # Keep only the last 30 frames of samples for a rolling average
            if len(samples) > 30:
              samples.pop(0)

            # Once we have enough samples, lock onto the median (best stable position) to kill jitter
            if len(samples) >= 15:
              interface_global_y = int(np.median(samples))
            else:
              prev_iy = tube_data["interface_y"]
              interface_global_y = int(
                  alpha * interface_global_y + (1 - alpha) * prev_iy
              )
          else:
            samples = [interface_global_y]
    ...
    cv2.imshow("Advanced Interface Detection", sharpened_frame)
